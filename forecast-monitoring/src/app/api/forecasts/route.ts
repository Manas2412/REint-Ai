...
export async function GET(request: Request) {
  ...
  // Use real WINDFOR data from stream API
  try {
    ...
    const rows = await fetchWindFor({ apiKey: apiKey!, startDate: fetchStart, endDate: fetchEnd });

    const parsed = rows
      .map((row) => {
        return {
          targetTime: row.StartTime,
    ...
    console.error("Error fetching WINDFOR data:", error);
    return NextResponse.json({ error: "Failed to fetch forecasts data" }, { status: 500 });
  }
}
